-- Daily OS — Supabase table setup v6
-- ─────────────────────────────────────────────────────────────
...
CREATE TABLE IF NOT EXISTS daily_checkins (
    ...
    type                TEXT            NOT NULL
                            CHECK (type IN ('morning', 'afternoon', 'quicklog')),

    -- Shared fields (morning + afternoon)
...
-- Uncomment and run ONLY the lines relevant to what you're missing.
-- Safe to run — ADD COLUMN IF NOT EXISTS won't error if already present.
-- ─────────────────────────────────────────────────────────────

-- From v2 (mood/energy notes, focus quality):
...
-- ALTER TABLE daily_checkins ADD COLUMN IF NOT EXISTS steps_yesterday INTEGER CHECK (steps_yesterday >= 0);

-- From v6 (weight):
-- ALTER TABLE daily_checkins ADD COLUMN IF NOT EXISTS weight NUMERIC(5,2) CHECK (weight BETWEEN 30 AND 300);
